...
// The GroupChatFunc type is an adapter to allow the use of ordinary
// function as GroupChat mutator.
type GroupChatFunc func(context.Context, *ent.GroupChatMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f GroupChatFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.GroupChatMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.GroupChatMutation", m)
}
...
func (f PrivateChatFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.PrivateChatMutation); ok {
		return f(ctx, mv)
	}
