...
// QueryPrivateChat chains the current query on the "private_chat" edge.
func (_q *ChatQuery) QueryPrivateChat() *PrivateChatQuery {
...
// QueryGroupChat chains the current query on the "group_chat" edge.
func (_q *ChatQuery) QueryGroupChat() *GroupChatQuery {
...
// WithPrivateChat tells the query-builder to eager-load the nodes that are connected to
// the "private_chat" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ChatQuery) WithPrivateChat(opts ...func(*PrivateChatQuery)) *ChatQuery {
	query := (&PrivateChatClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPrivateChat = query
	return _q
}

// WithGroupChat tells the query-builder to eager-load the nodes that are connected to
// the "group_chat" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ChatQuery) WithGroupChat(opts ...func(*GroupChatQuery)) *ChatQuery {
	query := (&GroupChatClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ChatQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Chat, error) {
	...
	if query := _q.withPrivateChat; query != nil {
		if err := _q.loadPrivateChat(ctx, query, nodes, nil,
			func(n *Chat, e *PrivateChat) { n.Edges.PrivateChat = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *ChatQuery) loadPrivateChat(ctx context.Context, query *PrivateChatQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *PrivateChat)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(privatechat.FieldChatID)
	}
	query.Where(predicate.PrivateChat(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(chat.PrivateChatColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *ChatQuery) loadGroupChat(ctx context.Context, query *GroupChatQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *GroupChat)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(groupchat.FieldChatID)
	}
	query.Where(predicate.GroupChat(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(chat.GroupChatColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
